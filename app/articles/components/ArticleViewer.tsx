...
export default function ArticleViewer({ articleId, galleys, files = [] }: ArticleViewerProps) {
  ...
  // Find matching file for selected galley
  const getFileUrl = (galley: Galley) => {
    if (galley.file?.url) return galley.file.url;
    if (galley.submissionFileId && files.length > 0) {
      const matchingFile = files.find(f => f.id === galley.submissionFileId);
  ...
  const fileUrl = selectedGalley ? getFileUrl(selectedGalley) : undefined;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* PDF Viewer */}
      {isPDF && viewMode === 'inline' && selectedGalley && fileUrl && (
